...
description: Find differentiated market angles, mechanisms, narratives, and tests that make an offer stand out. Triggers on positioning, differentiation, weak sales, crowded markets, mechanism naming, and angle selection.
license: MIT
metadata:
  author: genfeedai
  version: 1.0.0
...
| Genfeed copywriter | Write headlines and copy using the angle |
...
- If user needs to **build complete expert positioning** → `expert-architect`
- If user needs to **validate their positioning** → `expert-validator`
- If user needs to **build the offer** → `offer-architect`
- If user needs to **write copy** → Genfeed copywriter
- If user is **stuck on execution** → `execution-accelerator`
- If user needs to **validate market size** → `market-sizer`
