---
name: social-poster
description: The post stage of the content loop. Publishes a single derivative to X or LinkedIn over their REST APIs with global fetch (no SDK). Pure stateless worker that ENFORCES AN APPROVAL GATE — without --confirm it is a dry run that prints the exact request it would send and makes no network call. Reads its token from the environment, persists nothing. Triggers on "post to x", "post to twitter", "post to linkedin", "publish post", "social poster", "publish to socials", "post tweet".
license: MIT
metadata:
  author: genfeedai
  version: "1.0.0"
