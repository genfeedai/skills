---
name: trend-scout
description: The sense stage of the content loop. Pulls what's trending from public, mostly key-free sources (Hacker News, Reddit, Google Trends, generic RSS, optional NewsAPI), normalizes every hit to a scored TrendSignal, dedupes across sources, and emits a ranked JSON list. Pure stateless worker — no state, no secrets, stdin/stdout only. Triggers on "find trends", "what's trending", "trend scout", "trend ingestion", "scout trends", "trend candidates".
license: MIT
metadata:
  author: genfeedai
  version: "1.0.0"
